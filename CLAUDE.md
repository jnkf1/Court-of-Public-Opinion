...
│   │       └── ai_policy.html
│   ├── scripts/
│   │   ├── env.js             (BASE_URL constant; gitignored — local config)
│   │   ├── env.example.js     (tracked placeholder, matches server/ai/config.example.php's pattern)
│   │   ├── axios.min.js
│   │   ├── helper.js          (loadStoredData / saveStoredData / showNotification; also self-starts a 3s `getMyActiveRoom.php` poll on every page except room.html/courtroom.html, notifying a room host the moment someone joins — see Key Decisions below)
...
- **A host finds out someone joined their room no matter where they're browsing.** All three polling points (`room.js`, `courtroom.js`'s existing `checkActiveRoom()`, and a new self-starting poller in `helper.js` for every other page) track the room's previous status client-side and fire `showNotification("Someone joined your room! The debate has started.")` the instant they observe the transition from `open` to `in_progress`. This only ever fires for the host: a room can only be `open` while it has no joiner, so nobody but the host could have seen that prior state for "their" active room. `helper.js`'s poller is skipped on `room.html`/`courtroom.html` specifically to avoid double-polling and duplicate notifications on the two pages that already handle it themselves.
...
- **Forfeit is available in both debate modes and always resolves as a guaranteed loss — no AI judging involved.** A confirm-then-post `#forfeitBtn` sits below the chat input on both `debate.html` and `room.html`. Deliberately skips Gemini entirely (unlike normal judging) since a forfeit's outcome isn't actually in question — `forfeitAiDebate.php` inserts a fixed `LOST`/`0`-across-the-board row directly; `forfeitRoom.php` does the same guarded `status='in_progress' → 'closed'` + `affected_rows` race-check as `getRoomState.php`'s auto-close (so a forfeit racing the 1-min timer, or a double-click, can't double-insert), then inserts fixed `LOST`/`0` for the forfeiter and `WON`/`100` for the opponent. If the room's already closed by the time `forfeitRoom.php` runs, it just returns the caller's existing `cases` row instead of erroring. Forfeiting an `open` room (nobody's joined yet) is rejected with a clear message pointing at Cancel instead — forfeit only makes sense once a debate is actually underway. `#forfeitBtn` on `room.html` is disabled the whole time a room sits `open`, matching the chat input, so this path is mostly a defensive backstop rather than something a user hits through normal use.

## Next Steps (as of last session — 2026-08-26; **deadline is 2026-08-26 11:59 PM**)

All planned features are built and tested: AI debates (chat, judging, forfeit, resume-on-navigation, topic-debatability check), room debates (chat, judging, forfeit, cancel, return-to-your-debate from anywhere on the site, join notifications, topic-debatability check), auth/profile, court records, contact us, policies. The database was also wiped and reseeded once with 20 fresh users/rooms/cases (password `password123` on every seeded account) partway through tonight, at the user's request — anyone testing should expect that dataset, not whatever existed earlier.

**Final review pass (2026-08-26)**, checking the whole project end-to-end for unnecessary files and remaining bugs: no stray/unnecessary files were found (every debug/test script created during development stayed confined to the session's scratchpad, never landed in the actual project tree). Bugs found and fixed:
- `server/profile/getProfile.php` — `SUM()`/`AVG()` on a 0-case user returned SQL `NULL`, which rendered as blank Wins/Losses/Draws/Argument-Profile stats instead of `0` on a brand-new profile. Fixed with `COALESCE(..., 0)` on every aggregate.
- `client/scripts/room.js` + `server/rooms/forfeitRoom.php` — the Forfeit button was never disabled while a room sat `open` (waiting for an opponent), unlike the chat input which already was. Clicking it produced the misleading "This debate has already ended" from the backend. Fixed on both ends: the button now disables/enables in step with the chat input, and the backend gives a clear "hasn't started yet — cancel it instead" message as a defensive backstop.
- `client/scripts/env.js` — `BASE_URL` had a casing typo (`Court_of_Public_opinion`, lowercase o) that only worked because Windows/NTFS is case-insensitive; would 404 every request on a case-sensitive filesystem (Linux, most real hosting). Fixed, and added `env.example.js` as a tracked placeholder — `env.js` is gitignored and, unlike `config.php`, never had one, despite `.gitignore`'s own comment saying local config files should.
- `server/ai/config.example.php` — was fully commented out, making it useless as the setup template it exists to be. Uncommented.
- `client/pages/policies/privacy_policy.html` — Section 5 still described the original "deletion blocked if you have case history" behavior, which was reversed to a cascading delete early in development. Rewritten to match what `deleteAccount.php` actually does.
- `client/pages/policies/ai_policy.html` — Section 1 only described the AI's judging role; didn't mention it also plays the opponent in "Judge a Case" or that it now checks topic debatability. Expanded to cover all three roles.
...
3. Keep an eye on the Gemini free-tier quota during grading/demo — it ran dry at least once tonight from cumulative testing (confirmed via a direct `callGemini()` HTTP 429). If `gemini-3.6-flash` runs dry, switch to another Flash-family model name or a different Google Cloud project (see the AI provider decision above); this is an expected possibility, not a sign the integration is broken.
4. **Recommended before submission**: a full manual click-through of every page/flow in a real browser. Most of this session's real bugs (the BOM, the TDZ crash, the FK ordering, the disabled-button gap above) were only caught once actual browser/live testing happened, not from code review alone — worth doing one pass end-to-end (signup → daily case debate → forfeit/finish → profile record; create room → join from a second account → chat → forfeit/finish → court records) before calling it done.
