...
└── server/
    ...
    │   ├── getRoomState.php   (token-authed; caller must be a participant; lazily closes the room + judges it once 15 min have passed since started_at, see below; returns room details incl. computed joiner_stance, full message log, and myVerdict)
...
- **Real debate timer: 15 minutes, implemented for both AI debates and room debates.** Same limit for both. `room.html`/`room.js` + `getRoomState.php` enforce it for room debates (`rooms.started_at`, client-side countdown + server-side lazy auto-close). `debate.js` now has a matching client-side-only countdown (no `started_at` column needed there — nothing else needs to read it, unlike a room with two participants).
...
- **Judging/verdict is built and live for both modes.** Both `judgeAiDebate.php` and the judging logic inside `getRoomState.php` ask Gemini for **strict JSON only** (`{"verdict"/"winner": ..., ..._score: 0-100 each}`) via a dedicated system instruction — replies aren't guaranteed to be bare JSON (Gemini sometimes wraps it in prose or a code fence), so `extractJsonFromText()` in `server/ai/api.php` defensively finds the first `{` and last `}` and decodes that substring rather than trusting `json_decode()` on the raw text.
  - **AI debates** are judged client-side-triggered: `debate.js`'s 15-minute countdown calls `judgeDebate()` itself when it hits zero, which posts to `judgeAiDebate.php`. One `cases` row is inserted (`opponent_id`/`room_id` both `NULL`).
  - **Room debates** are judged server-side, embedded in `getRoomState.php`'s existing lazy 15-minute auto-close check — no separate endpoint or frontend trigger. Two `cases` rows are inserted (one per participant, sharing `room_id`, each `opponent_id` pointing at the other), verdict computed per side (WON/LOST split, or DRAW/DRAW).
